...
const UI_CONFIG: { [K in keyof typeof DotaDefaultUIElement_t]: boolean } = {
  // [0] Time of day (clock)
...
class CustomUIManifest extends Component<Elements> {
  constructor() {
    super();
    ...
    CustomUIConfig.COMBOS = new CombosCollection();

    CustomUIConfig.HERO_DATA = new NetTableListener(
      NetTableName.Invokation,
      NetTableKey.InvokationHeroData
    );

    CustomUIConfig.HERO_KV = new NetTableListener(NetTableName.Hero, NetTableKey.HeroKeyValues);

    CustomUIConfig.ABILITIES_KV = new NetTableListener(
      NetTableName.Abilities,
      NetTableKey.AbilitiesKeyValues
    );
...
const component = new CustomUIManifest();
