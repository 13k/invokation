...
import type * as TCustomNetTables from "../../lib/custom_net_tables";
...
class TooltipStatBranch extends Component<Elements> {
  selected?: number;
  selectedSplit?: Dota2.TalentMap<boolean>;
  heroData?: TCustomNetTables.HeroData;
  talents?: Dota2.TalentMap<Dota2.Ability>;
  rows: Record<Dota2.TalentLevel, Panel>;
  ...
  constructor() {
    ...
    this.rows = {} as Record<Dota2.TalentLevel, Panel>;
  ...
  onLoad() {
    this.selected = this.panel.GetAttributeUInt32("selected", 0);
    this.selectedSplit = splitSelectedTalents(this.selected);

    this.debug("onLoad()", { selected: this.selected, selectedSplit: this.selectedSplit });
    this.render();
  }

  onHeroDataChange(data: NetworkedData<TCustomNetTables.HeroData>) {
    ...
    this.talents = _.transform(
      ...
      (abilities, ability, i) => {
        ...
        const side = talentArrayIndexToSide(i);

        abilities[level] = abilities[level] || {};
        abilities[level][side] = ability;
      },
      {} as Dota2.TalentMap<Dota2.Ability>
    );
  ...
  localizeBranch(panel: Panel, level: Dota2.TalentLevel, side: Dota2.TalentSide) {
    ...
    this.debug("localizeBranch() : FindChildrenWithClassTraverse [label]", { branchLabelClass });
  ...
  selectBranchAction(level: Dota2.TalentLevel) {
    ...
    seq = seq.Function(() => this.debug("selectBranchAction()", { level: level, side: side }));
...
const component = new TooltipStatBranch();
