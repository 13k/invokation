...
const {
  ...
  Sequence: { Sequence, StopSequence, ParallelSequence, AddOptionAction, RunFunctionAction },
  Util: { pascalCase },
...
class TagSelect extends Component<Elements> {
  options: string[];
  tags: string[];
  ...
  setOptions(payload: { options: string[] }) {
  ...
  onPopupTextEntrySubmit(payload: TCustomEvents.PopupTextEntrySubmit) {
  ...
  createOptionAction(option: string) {
    return new AddOptionAction(this.elements.options, _.bind(this.createOption, this, option));
  }
  ...
  clearTagsAction() {
    const actions = _.map(this.tags, (tag) => this.removeTagAction(tag, { immediate: true }));
  ...
  showTagEntryPopup() {
    this.showPopup(this.elements.options, Layout.ID.PopupTextEntry, PanelID.PopupTextEntry, {
