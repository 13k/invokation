...
import { ComponentLayout } from "./const/component";
...
export abstract class Component {
  ...
  */
  ...
  ): PanelWithComponent<Component> {
    const panel = this.createPanel(parent, id, { ...options, layout: componentLayout(layout) });
    const component = getPanelComponent<Component>(panel);

    this.applyComponentOptions(component, options);

    return panel as PanelWithComponent<Component>;
  }
  ...
  loadComponent<T extends Panel, Layout extends ComponentLayout>(
    ...
    return panel as WithComponent<T, Component>;
  }
