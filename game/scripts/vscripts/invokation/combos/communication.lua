...
local function comboMetrics(combo)
  return {count = combo.count, damage = combo.damage, duration = combo.duration}
end
...
function M.sendStarted(player, combo)
  local payload = {id = combo.id, next = combo:NextStepsIds()}
...
function M.sendProgress(player, combo)
  local payload = {id = combo.id, metrics = comboMetrics(combo), next = combo:NextStepsIds()}
...
function M.sendStepError(player, combo, ability)
  local payload = {id = combo.id, expected = combo:NextStepsIds(), ability = ability.name}
...
function M.sendPreFinish(player, combo)
  local payload = {id = combo.id, metrics = comboMetrics(combo), wait = combo.waitDuration}
...
function M.sendFinished(player, combo)
  local payload = {id = combo.id, metrics = comboMetrics(combo)}
