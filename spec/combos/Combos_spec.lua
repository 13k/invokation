...
describe("Combos", function()
  ...
  before_each(function()
    ...
    combos = Combos({ netTable = netTable })
  end)
  ...
  describe("constructor", function()
    it("loads and transforms specs", function()
      assert.are.equal(m.count(COMBOS), m.count(combos.specs))
    ...
    it("sets NetTable key with loaded specs", function()
      assert.stub(netTable.Set).was.self.called_with(NET_TABLE.MAIN_KEYS.COMBOS, combos.specs)
    end)
  end)
  ...
  describe("#Start", function()
    ...
    describe("when no combo is active", function()
      it("starts the combo", function()
        ...
        assert.spy(CombosComm.sendStarted).was.called_with(
          match.is_ref(player),
          match.is_ref(combo)
        )
    ...
    describe("when the same combo is already active", function()
      it("restarts the combo", function()
        ...
        assert.is_not_nil(comboBefore)
        assert.are.equal(13, comboBefore.id)
        ...
        assert.is_not_nil(comboAfter)
        assert.is_not_nil(dummy)

        assert.are_not.equal(comboAfter, comboBefore)
    ...
    describe("when a different combo is already active", function()
      it("stops the previous combo and starts the new combo", function()
        ...
        assert.are_not.equal(comboAfter, comboBefore)
        assert.are.equal(3, comboAfter.id)
        assert.is_true(dummy:IsAlive())
        ...
        assert.spy(CombosComm.sendStopped).was.called_with(
          match.is_ref(player),
  ...
  describe("#Stop", function()
    ...
    describe("when a combo is active", function()
      it("stops the active combo", function()
        ...
        assert.is_not_nil(dummyAfter)
        assert.are.equal(dummyBefore, dummyAfter)
    ...
    describe("when a combo is active", function()
      describe("without hard reset option", function()
        it("restarts the combo", function()
          ...
          assert.spy(CombosHero.teardown).was.called_with(
            match.is_ref(player),
            { hardReset = false }
          )

          assert.spy(CombosHero.setup).was.called_with(
            match.is_ref(player),
            match.is_ref(comboAfter)
          )
    ...
    describe("with regular active combo", function()
      ...
      describe("with correct ability", function()
        it("progresses the combo", function()
          ...
          assert.spy(combos.PreFinish).was_not.called()
          assert.spy(combos.Fail).was_not.called()
          assert.spy(CombosComm.sendProgress).was.called_with(
            match.is_ref(player),
            match.is_ref(combo)
          )
        end)
        ...
        describe("when it's the first combo step", function()
          it("communicates that combo is in progress", function()
            ...
            assert.spy(CombosComm.sendInProgress).was.called_with(
              match.is_ref(player),
              match.is_ref(combo)
            )
          end)
