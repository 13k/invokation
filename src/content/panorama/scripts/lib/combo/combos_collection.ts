import type { Combo, ComboId, Properties, Step } from "../combo";
  ...
  protected override normalize(rvalue: NetworkValue): Value {
    ...
    for (const rcombo of Object.values(rvalue)) {
      ...
      const props: Properties = {
        specialty: rcombo.specialty,
        stance: rcombo.stance,
        damageRating: rcombo.damage_rating,
        difficultyRating: rcombo.difficulty_rating,
      };

      const combo: Combo = {
        id: rcombo.id,
        ...props,
        heroLevel: rcombo.hero_level,
        orbs,
        orbsByName,
        talents: rcombo.talents,
        items,
        tags,
        tagset,
        sequence,
        l10n: {
          ...l10n.comboProps(props),
